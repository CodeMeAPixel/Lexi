...
const TokenResetSchema = z.object({ token: z.string().min(1), email: z.string().email(), password: z.string().min(8) });
const AuthResetSchema = z.object({ currentPassword: z.string().min(1), password: z.string().min(8) });
...
export async function POST(req: Request) {
    try {
        ...
        if (!user || !user.hashedPassword) return NextResponse.json({ error: 'User not found' }, { status: 404 });
