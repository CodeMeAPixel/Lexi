import React from "react";
import Link from "next/link";

export const dynamic = "force-dynamic";

export default function AdminMainPage() {
  return (
    <main className="w-full max-w-5xl mx-auto">
      <section className="rounded-2xl border border-white/10 bg-white/[0.02] p-6 sm:p-8">
        <h1 className="mb-6 text-3xl font-semibold tracking-tight">Admin Dashboard</h1>
        <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
          <Link
            href="/admin/users"
            className="flex flex-col gap-2 p-6 transition-colors rounded-xl border border-white/10 bg-white/[0.02] hover:bg-white/[0.05]"
          >
            <span className="text-lg font-semibold">User Manager</span>
            <span className="text-sm text-white/65">
              View, edit, and manage users and roles.
            </span>
          </Link>
          <Link
            href="/admin/activities"
            className="flex flex-col gap-2 p-6 transition-colors rounded-xl border border-white/10 bg-white/[0.02] hover:bg-white/[0.05]"
          >
            <span className="text-lg font-semibold">
              Audit Log / Activities
            </span>
            <span className="text-sm text-white/65">
              Review all user and system activities.
            </span>
          </Link>
          {/* Add more admin panels here as needed */}
        </div>
      </section>
    </main>
  );
}
